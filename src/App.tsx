...
const URL =
	"wss://sszw8t4j4k.execute-api.eu-central-1.amazonaws.com/production";
...
const App = () => {
	...
	const onSocketOpen = useCallback(() => {
		...
		socket.current?.send(JSON.stringify({ action: "setDisplayName", name }));
	}, []);
	...
	const onConnect = useCallback(() => {
		if (socket.current?.readyState !== WebSocket.OPEN) {
			...
			socket.current.addEventListener("message", (event: { data: any }) => {
	...
	const onSendPrivateMessage = useCallback((destination: string) => {
		const message = prompt("Enter private message for " + destination);
		socket.current?.send(
			JSON.stringify({
